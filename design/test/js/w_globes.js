...
var globes = function() {
    ...
    function fitProjection(proj, view) {
        var _d3$geoPath$projectio = d3.geoPath().projection(proj).bounds({
            type: "Sphere"
        }),
            _d3$geoPath$projectio2 = _slicedToArray(_d3$geoPath$projectio, 2),
            _d3$geoPath$projectio3 = _slicedToArray(_d3$geoPath$projectio2[0], 2),
            x0 = _d3$geoPath$projectio3[0],
            y0 = _d3$geoPath$projectio3[1],
            _d3$geoPath$projectio4 = _slicedToArray(_d3$geoPath$projectio2[1], 2),
            x1 = _d3$geoPath$projectio4[0],
            y1 = _d3$geoPath$projectio4[1];

        var hScale = (x1 - x0) / proj.scale();
        var vScale = (y1 - y0) / proj.scale();
    ...
    function standardGlobe() {
        return {
            ...
            bounds: function bounds(view) {
                var _d3$geoPath$projectio5 = d3.geoPath().projection(this.projection).bounds({
                    type: "Sphere"
                }),
                    _d3$geoPath$projectio6 = _slicedToArray(_d3$geoPath$projectio5, 2),
                    _d3$geoPath$projectio7 = _slicedToArray(_d3$geoPath$projectio6[0], 2),
                    x0 = _d3$geoPath$projectio7[0],
                    y0 = _d3$geoPath$projectio7[1],
                    _d3$geoPath$projectio8 = _slicedToArray(_d3$geoPath$projectio6[1], 2),
                    x1 = _d3$geoPath$projectio8[0],
                    y1 = _d3$geoPath$projectio8[1];

                var xMin = Math.max(Math.floor(ensureNumber(x0, 0)), 0);
                var yMin = Math.max(Math.floor(ensureNumber(y0, 0)), 0);
                var xMax = Math.min(Math.ceil(ensureNumber(x1, view.width)), view.width - 1);
                var yMax = Math.min(Math.ceil(ensureNumber(y1, view.height)), view.height - 1);
                return {
                    xMin: xMin,
                    yMin: yMin,
                    xMax: xMax,
    ...
    function orthographic(view) {
        return newGlobe({
            newProjection: function newProjection(view) {
                return fitProjection(Object(newOrthographic)().rotate(currentPosition()).precision(0.1).clipAngle(90), view);
            },
                ...
                return {
                    renderTo: function renderTo(context, path) {
                        ...
                        if (theme == 'light') {
                            ...
                            gradient.addColorStop(0.97, "rgba(0, 0, 0, 0.3)");
                            gradient.addColorStop(0.999, "rgba(0, 0, 0, 0.4)");
                        ...
                        context.beginPath();
                        if (theme == 'light') {
                            context.arc(x, y, scale - 0.5, 0, τ, true);
                            context.fillStyle = "rgba(255, 255, 255, 0.8)";
                        } else {
                            context.arc(x, y, scale - 1, 0, τ, true);
...
function newOrthographic() {
    return Object.assign(d3.geoOrthographic(), {
        ...
        optimize: function optimize() {
            return orthographicProjection.apply(void 0, _toConsumableArray(Object(orthographicArgsFromD3)(this)));
        }
    });
}
